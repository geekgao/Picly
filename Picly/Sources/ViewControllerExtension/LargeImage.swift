...
//            }
    ...
    /// 获取当前目录按展示顺序排列的文件列表（AI 模式下按得分，否则按 Map 排序）
    private func displayFilesForCurrentFolder() -> (curFolder: String, files: [FileModel]) {
        fileDB.lock()
        let curFolder = fileDB.curFolder
        defer { fileDB.unlock() }
        guard let dirModel = fileDB.db[SortKeyDir(curFolder)] else {
            return (curFolder, [])
        }
        if dirModel.isFiltered && !dirModel.aiOrderedPaths.isEmpty {
            return (curFolder, dirModel.filesForDisplay())
        }
        return (curFolder, dirModel.files.map { $0.1 })
    }
    ...
    func locateLargeImage(direction: Int, isShowReachEndPrompt: Bool = true, firstShowThumb: Bool = true, noLoopBrowsing: Bool = false){
        ...
        let (curFolder, displayFiles) = displayFilesForCurrentFolder()
        let totalCount = displayFiles.count
        ...
        if direction == -1 {
            while nextLargeImagePos >= 0 {
                nextLargeImagePos-=1
                if nextLargeImagePos >= 0, nextLargeImagePos < totalCount {
                    let file = displayFiles[nextLargeImagePos]
        ...
        // Backward
            ...
            while nextLargeImagePos < totalCount-1 {
                nextLargeImagePos+=1
                if nextLargeImagePos < totalCount {
                    let file = displayFiles[nextLargeImagePos]
                    if file.type == .image || (file.type == .video && globalVar.useInternalPlayer) {
                        ifFoundNextImage=true
                        break
        ...
        if ifFoundNextImage {
            ...
            if globalVar.portableMode {
                let refSize = (nextLargeImagePos >= 0 && nextLargeImagePos < displayFiles.count) ? displayFiles[nextLargeImagePos].originalSize : nil
    ...
    func preloadLargeImage(){
        ...
        let (_, displayFiles) = displayFilesForCurrentFolder()
        let totalCount = displayFiles.count
        guard currLargeImagePos < totalCount else { return }
        let currentFile = displayFiles[currLargeImagePos]
        guard let url = URL(string: currentFile.path) else { return }
        ...
        nextLargeImagePos=currLargeImagePos
        loadCount=0
        while nextLargeImagePos >= 0 {
            nextLargeImagePos -= 1
            if nextLargeImagePos < 0 { break }
            let file = displayFiles[nextLargeImagePos]
            if file.type == .image || (file.type == .video && globalVar.useInternalPlayer) {
                loadCount += 1
        ...
        if currentFile.type == .image {
            fileQueue.append((currentFile, 0))
        }
    ...
    func changeLargeImage(firstShowThumb: Bool = true, resetSize: Bool = true, triggeredByLongPress: Bool = false, justChangeLargeImageViewFile: Bool = false, forceRefresh: Bool = false, isByZoom: Bool = false){
        ...
        if publicVar.openFromFinderPath != "" {
            ...
            let (_, displayFiles) = displayFilesForCurrentFolder()
            if pos >= 0 && pos < displayFiles.count {
                file = displayFiles[pos]
            }
        ...
        if var originalSize=originalSize{
            ...
            NSLog("[DIAG] openImage largeSize=\(Int(largeSize.width))x\(Int(largeSize.height)) orig=\(Int(originalSize.width))x\(Int(originalSize.height)) scale=\(scale) doNotResize=\(doNotGenResized) ver=\(file.ver)")
            ...
            if firstShowThumb && !isImageCached {
                var thumb = file.image
                if thumb == nil || thumb!.size.width < 200 {
                    thumb = getImageThumb(url: url, refSize: originalSize)
                }
                NSLog("[DIAG] show initial thumb: \(thumb != nil ? "image" : "nil") size=\(thumb?.size.width ?? 0)x\(thumb?.size.height ?? 0)")
                largeImageView.imageView.image = thumb?.rotated(by: CGFloat(-90*rotate))
            }
            ...
            if file.type == .image {
                ...
                task = DispatchWorkItem { [weak self] in
                    NSLog("[DIAG] task body START")
                    guard let self = self else { NSLog("[DIAG] task body: self is nil, return"); return }
                    if pos != currLargeImagePos && !isThisFromFinder { NSLog("[DIAG] task body: pos changed, return"); return }
                    
                    largeImageLoadQueueLock.lock()
                    
                    if task?.isCancelled ?? false {
                        log("1 - Load large image replace task was cancelled.")
                        NSLog("[DIAG] task body: cancelled at check 1")
                    ...
                    if resetSize && !forceRefresh {
                        ...
                        NSLog("[DIAG] getImageCache returned \(largeImage != nil ? "image" : "nil") size=\(largeImage?.size.width ?? 0)x\(largeImage?.size.height ?? 0)")
                    }else{
                        if isHDR {
                        ...
                        NSLog("[DIAG] alt path returned \(largeImage != nil ? "image" : "nil") size=\(largeImage?.size.width ?? 0)x\(largeImage?.size.height ?? 0)")
                    }
                    ...
                    if largeImage != nil{
                        ...
                        func doReplace() {
                            ...
                            NSLog("[DIAG] doReplace called, setting image size=\(largeImage?.size.width ?? 0)x\(largeImage?.size.height ?? 0)")
                ...
                NSLog("[DIAG] taskDispatch: isLaunchFromFile=\(publicVar.isLaunchFromFile_changeLargeImage)")
                if publicVar.isLaunchFromFile_changeLargeImage {
                    // 直接执行任务（当前已在主线程），避免死锁
                    // Execute task directly (currently on main thread) to avoid deadlock
                    NSLog("[DIAG] taskDispatch: calling perform()")
                    task!.perform()
                    NSLog("[DIAG] taskDispatch: perform() done")
                    publicVar.isLaunchFromFile_changeLargeImage = false
                } else {
                    NSLog("[DIAG] taskDispatch: calling async global queue")
                    DispatchQueue.global(qos: .userInitiated).async(execute: task!)
                    NSLog("[DIAG] taskDispatch: async dispatched")
                }
