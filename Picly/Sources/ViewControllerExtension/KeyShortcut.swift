...
extension ViewController {
    ...
    func KeyShortcutManager (event: NSEvent) -> NSEvent?
    {
        ...
        // Cmd+= / Cmd+- 缩放/缩略图大小 (Chrome 风格, 优先处理避免被菜单拦截)
        // Cmd+= / Cmd+- zoom (Chrome style, process early to avoid menu interception)
        if isCommandPressed && !isAltPressed && !isCtrlPressed {
            let keyCode = event.keyCode
            if (keyCode == 24 || keyCode == 69 || characters == "=" || characters == "+") {
                if publicVar.isInLargeView {
                    ...
                    changeThumbSizeUntilColumnChange(direction: +1)
                }
                return nil
            }
            if (keyCode == 27 || keyCode == 78 || characters == "-") {
                if publicVar.isInLargeView {
                    if largeImageView.file.type == .video {
        ...
        if publicVar.isKeyEventEnabled {
            ...
            if (isOnlyCommandPressed && characters == "w") || event.keyCode == 53 {
                ...
                // Esc 清除当前活跃的过滤条件
                // Esc clears the currently active filter
                if !publicVar.finderTagFilters.isEmpty && searchOverlay == nil {
                    handleClearFinderTagFilter()
                    coreAreaView.showInfo(NSLocalizedString("Tag filter cleared", comment: "标签过滤已清除"), timeOut: 1.5)
                    return nil
                }
                if !publicVar.ratingFilters.isEmpty && searchOverlay == nil {
                    handleClearRatingFilter()
                    coreAreaView.showInfo(NSLocalizedString("Rating filter cleared", comment: "评级过滤已清除"), timeOut: 1.5)
                    return nil
                }
    ...
    private func changeThumbSizeUntilColumnChange(direction: Int) {
        let step = 16
        let rounded = ((publicVar.profile.thumbSize + step / 2) / step) * step
        var newSize = rounded
        ...
        while lastWaterFallNumberOfColumns == publicVar.waterfallLayout.numberOfColumns {
            newSize += direction * step
            newSize = max(256, min(1200, newSize))
            guard newSize != publicVar.profile.thumbSize else { return }
            publicVar.profile.thumbSize = newSize
        ...
        for item in collectionView.visibleItems() {
            (item as? CustomCollectionViewItem)?.setCustomFrameSize()
        }
    }
}
