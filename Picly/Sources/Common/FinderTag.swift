...
class EnhancedIndex {
    ...
    private static let saveQueue = DispatchQueue(label: "com.picly.EnhancedIndex.save")
    private static let debounceInterval: TimeInterval = 2.0

    private static let scanQueue = DispatchQueue(label: "com.picly.EnhancedIndex.scan")
    ...
    private static var dataFileURL: URL {
        ...
        let piclyDir = appSupport.appendingPathComponent("Picly")
        try? FileManager.default.createDirectory(at: piclyDir, withIntermediateDirectories: true)
        return piclyDir.appendingPathComponent("EnhancedIndex.json")
    }
    ...
    static func updateFiles(_ urls: [URL], isCalledByDirOpen: Bool = false, recordTime: Bool = false, skipTagRead: Bool = false) {
        guard globalVar.enhancedIndexEnabled else { return }
        if !isLoaded {
        ...
        for url in urls {
            ...
            // 无标签过滤需求时，跳过所有 tagNames 磁盘 I/O
            // When no tag filter is active, skip all tagNames disk I/O
            if skipTagRead { continue }
        ...
        if recordTime {
            ...
            log("EnhancedIndex: updateFiles(\(urls.count) urls, changed=\(changed), skipTagRead=\(skipTagRead)) in \(String(format: "%.4f", elapsed))s", level: .info)
        }
    }
