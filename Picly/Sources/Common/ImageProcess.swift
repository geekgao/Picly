...
/// 从 JPEG/PNG 文件头快速读取像素尺寸和 EXIF 旋转方向，
/// 比 CGImageSource 少读取大量文件元数据，减少 I/O。
private func fastImageSizeFromHeader(url: URL) -> (size: NSSize, orientation: Int)? {
    ...
    if ext == "png" {
        let data = fh.readData(ofLength: 33)
...
private func parseJPEGDimensions(_ data: Data) -> (size: NSSize, orientation: Int)? {
    let b = [UInt8](data)
    guard b.count >= 4, b[0] == 0xFF, b[1] == 0xD8 else { return nil }
    var orientation = 1
    var idx = 2
    while idx + 4 < b.count {
        guard b[idx] == 0xFF else { return nil }
        let marker = b[idx + 1]
        let segLen = Int(b[idx + 2]) << 8 | Int(b[idx + 3])
        if marker == 0xC0 || marker == 0xC2 {
            guard idx + 9 < b.count else { return nil }
            let h = Int(b[idx + 5]) << 8 | Int(b[idx + 6])
            let w = Int(b[idx + 7]) << 8 | Int(b[idx + 8])
            let rotated = orientation >= 5 && orientation <= 8
            return (NSSize(width: rotated ? h : w, height: rotated ? w : h), orientation)
        }
        if marker == 0xE1, let orient = parseEXIFOrientationInJPEG(b, offset: idx + 4) {
            orientation = orient
        }
        idx += 2 + segLen
    }
...
private func parseEXIFOrientationInJPEG(_ b: [UInt8], offset: Int) -> Int? {
    guard offset + 14 < b.count,
          b[offset] == 0x45, b[offset+1] == 0x78,
          b[offset+2] == 0x69, b[offset+3] == 0x66,
          b[offset+4] == 0x00, b[offset+5] == 0x00 else { return nil }

    let tiff = offset + 6
    let isLE = b[tiff] == 0x49 && b[tiff+1] == 0x49
    let isBE = b[tiff] == 0x4D && b[tiff+1] == 0x4D
    guard isLE || isBE, tiff + 8 < b.count else { return nil }

    func r16(_ o: Int) -> Int {
        isLE ? Int(b[o]) | (Int(b[o+1]) << 8) : (Int(b[o]) << 8) | Int(b[o+1])
    }
    func r32(_ o: Int) -> Int {
        isLE
    ...
    for i in 0..<count {
        ...
        guard entry + 12 < b.count else { break }
        if r16(entry) == 0x0112 {
            return r16(entry + 8)
        }
    }
...
private func parsePNGDimensions(_ data: Data) -> (size: NSSize, orientation: Int)? {
    let b = [UInt8](data)
    guard b.count >= 24,
    ...
    return (NSSize(width: w, height: h), 1)
}
...
func getImageInfo(url: URL, needMetadata: Bool) -> ImageInfo? {
    ...
    if globalVar.HandledVideoExtensions.contains(url.pathExtension.lowercased()) {
        // 优先用 ffmpeg 读取视频尺寸（读取数据少），失败再回退到 AVAsset
        // Prefer ffmpeg for video dimensions (reads less data), fall back to AVAsset
        ...
        // 快速读取 JPEG/PNG 文件头尺寸，比 CGImageSource 更快
        // Fast path: read dimensions from JPEG/PNG file header
        let fastSizeResult = fastImageSizeFromHeader(url: url)

        if let (fastSize, _) = fastSizeResult, !needMetadata {
            let imageInfo = ImageInfo(fastSize)
            imageInfo.ext = url.pathExtension.lowercased()
            return imageInfo
        }
        ...
        if let (fastSize, _) = fastSizeResult {
            imageSize = fastSize
        } else {
            switch orientation {
            case 1, 2, 3, 4:
...
/// Lightweight GPS extraction from image file.
/// Reads only GPS EXIF dictionary — does not decode pixels.
/// - Returns: (latitude, longitude) in decimal degrees, or nil.
func extractGPS(from url: URL) -> (lat: Double, lon: Double)? {
    guard globalVar.HandledImageAndRawExtensions.contains(url.pathExtension.lowercased()),
