...
class CustomCollectionViewManager: NSObject, NSCollectionViewDataSource, NSCollectionViewDelegate, NSCollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: NSCollectionView, itemForRepresentedObjectAt indexPath: IndexPath) -> NSCollectionViewItem {
        ...
        fileDB.lock()
        if let dirModel = fileDB.db[SortKeyDir(fileDB.curFolder)] {
            if dirModel.isFiltered && !dirModel.aiOrderedPaths.isEmpty && indexPath.item < dirModel.aiOrderedPaths.count {
                let targetPath = dirModel.aiOrderedPaths[indexPath.item]
                if let file = dirModel.files.first(where: { $0.1.path == targetPath })?.1 {
                    item.configureWithImage(file)
                }
            } else if let file = dirModel.files.elementSafe(atOffset: indexPath.item)?.1 {
                item.configureWithImage(file)
            }
        }
        fileDB.unlock()
...
//                viewController.publicVar.selectedUrls2.append(URL(string: file.path)!)
//            }
//            fileDB.unlock()
        // }
...
//                if let index=viewController.publicVar.selectedUrls2.firstIndex(of: URL(string: file.path)!){
//                    viewController.publicVar.selectedUrls2.remove(at: index)
//                }
//            }
//            fileDB.unlock()
        // }
