...
extension CustomOutlineViewManager: NSOutlineViewDelegate {
    ...
    func outlineView(_ outlineView: NSOutlineView, viewFor tableColumn: NSTableColumn?, item: Any) -> NSView? {
        ...
        if treeNode.fullPath.contains("PiclyTitleFolder") {
            ...
            let folderIcon = NSImage(systemSymbolName: "folder.fill", accessibilityDescription: nil)?.withSymbolConfiguration(NSImage.SymbolConfiguration(pointSize: 18, weight: .regular))
            folderIcon?.isTemplate = true
            view.imageView?.image = folderIcon
            view.imageView?.contentTintColor = .systemBlue
        }
