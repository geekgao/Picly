...
//        }
    ...
    private func createEdgeArrowViews() {
        ...
        leftArrowImageView?.alphaValue = 0.3
        ...
        rightArrowImageView?.alphaValue = 0.3
        ...
        if let leftImageView = leftArrowImageView {
            ...
            let leftClick = NSClickGestureRecognizer(target: self, action: #selector(actArrowPrevious))
            leftImageView.addGestureRecognizer(leftClick)
        }
        if let rightImageView = rightArrowImageView {
            addSubview(rightImageView, positioned: .above, relativeTo: nil)
            let rightClick = NSClickGestureRecognizer(target: self, action: #selector(actArrowNext))
            rightImageView.addGestureRecognizer(rightClick)
        }
    }

    @objc private func actArrowPrevious() {
        getViewController(self)?.previousLargeImage()
    }

    @objc private func actArrowNext() {
    ...
    private func hideArrowView(_ imageView: NSImageView?, animated: Bool = true) {
        ...
        if animated {
            NSAnimationContext.runAnimationGroup({ context in
                context.duration = 0.2
                imageView.animator().alphaValue = 0.3
            }) {
                imageView.isHidden = false
            }
        } else {
...
//        }
    ...
    @objc func actSetAsWallpaper() {
        ...
        do {
            ...
            log("Failed to set wallpaper: \(error.localizedDescription)")
            let alert = NSAlert()
            alert.messageText = NSLocalizedString("set-wallpaper-fail", comment: "设置壁纸失败")
