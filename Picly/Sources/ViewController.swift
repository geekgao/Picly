...
class PublicVar{
    ...
    var isAIFilterOn = false
    var aiFilterPaths: [String] = []
    var aiIsSearching = false

    // 地理位置搜索
    var isGeoFilterOn = false
...
class ViewController: NSViewController, NSSplitViewDelegate, NSSearchFieldDelegate {
    ...
    var readInfoOperationQueue: OperationQueue?
    ...
    var watchEventStream: FSEventStreamRef?
    ...
    var searchAIModeButton: NSButton?
    var searchFilterButton: NSButton?
    var searchFullPathCheckbox: NSButton?
    ...
    // 刷新时保存的滚动位置，防止新增文件导致自动跳转
    // Saved scroll position during refresh, prevents auto-jump when files added
    var refreshSavedScrollPos: CGPoint? = nil

    // 性能调试
    // Performance debugging
    var firstGetImageInfoLogged = false
    ...
    // AI 语义搜索
    // AI semantic search
    var search_isAIMode: Bool = false
    var search_aiDebounceTask: Task<Void, Never>?

    // 地理位置搜索
    var search_isGeoMode: Bool = false
    var search_geocoder: CLGeocoder?
...
//        collectionView.addGestureRecognizer(clickCollectionItemGesture)
    ...
    func prepareForDeinit() {
        ...
        loadImageTaskPool.isTerminated = true
        willTerminate=true
...
//        }
    ...
    func startBackgroundTaskThread(){
        ...
        let readInfoThread = Thread { [weak self] in
            ...
            readInfoOperationQueue = operationQueue
            while true{
...
//                    }
                    ...
                    if(i == count-1){
                        operationQueue.waitUntilAllOperationsAreFinished()
                        // 全部读完后持久化缓存，防止 Debug 进程被杀死时缓存丢失
                        ImageSizeDiskCache.shared.saveToDisk()
                    }
                    ...
                    operationQueue.addOperation { [weak self] in
                        ...
                        let curFolder=fileDB.curFolder
                        let wasAlreadyProcessed = originalSize != nil && file.canBeCalcued
                        fileDB.unlock() 
                        ...
                        if originalSize == nil {
                            if doNotActualRead {
                                ...
                                // 尝试从磁盘缓存读取图像尺寸
                                var cachedSize: ImageSizeDiskCache.ImageSizeEntry? = nil
                                if let md = file.modDate {
                                    cachedSize = ImageSizeDiskCache.shared.get(filePath: key.path, modDate: md)
                                }
                                if let c = cachedSize {
                                    if c.isFail {
                                        originalSize = DEFAULT_SIZE
                                        ...
                                        originalSize = NSSize(width: c.width, height: c.height)
                                        imageInfo = ImageInfo(originalSize)
                                    }
                                } else {
                                    if !firstGetImageInfoLogged {
                                        firstGetImageInfoLogged = true
                                        let elapsed = Double(DispatchTime.now().uptimeNanoseconds - startTime.uptimeNanoseconds) / 1_000_000_000
                                        log("Time to first getImageInfo: \(elapsed) seconds")
                                    }
                                    // 外置卷首次只读文件头获取尺寸（JPEG 8KB, PNG 33 bytes），
                                    // 跳过 CGImageSource 全解析（慢 10×）。metadata 在需要时
                                    // 延迟读取（大图浏览、EXIF 排序等场景）。
                                    let isExternal = VolumeManager.shared.isExternalVolume(key.path)
                                    imageInfo = getImageInfo(url: URL(string: key.path)!, needMetadata: !isExternal)
                                ...
                                // 写入磁盘缓存
                                if let md = file.modDate, let sz = originalSize {
                                    ImageSizeDiskCache.shared.set(filePath: key.path, modDate: md, entry: ImageSizeDiskCache.ImageSizeEntry(width: sz.width, height: sz.height, isFail: isGetImageSizeFail))
                                }
                            }
                        }

                        // Extract GPS from image metadata (background thread)
                        if let info = imageInfo, let props = info.properties,
                           let gpsDict = props[kCGImagePropertyGPSDictionary as String] as? [String: Any],
                           let rawLat = gpsDict[kCGImagePropertyGPSLatitude as String] as? Double,
                           let rawLon = gpsDict[kCGImagePropertyGPSLongitude as String] as? Double,
                        ...
                        {
                            let lat = latRef == "S" ? -rawLat : rawLat
                            let lon = lonRef == "W" ? -rawLon : rawLon
                            file.gpsLatitude = lat
                            file.gpsLongitude = lon
                            GeoCache.shared.addFile(path: key.path, lat: lat, lon: lon)
                        } else {
                            // No GPS in cached properties — lightweight GPS-only read
                            // Runs for all cases: cache hit, external volumes, wasAlreadyProcessed
                            if let gps = extractGPS(from: URL(string: key.path)!) {
                                file.gpsLatitude = gps.lat
                                file.gpsLongitude = gps.lon
                                GeoCache.shared.addFile(path: key.path, lat: gps.lat, lon: gps.lon)
                            }
                        }
                        ...
                        if wasAlreadyProcessed {
                            if i == count - 1 {
                                DispatchQueue.main.async { [weak self] in
                                    guard let self = self else { return }
                                    fileDB.lock()
                                    dirModel.layoutCalcPos = dirModel.files.count
                                    dirModel.lastLayoutCalcPosUsed = dirModel.files.count
                        ...
                        if originalSize != nil {
                            ...
                            DispatchQueue.main.async { [weak self] in
                                ...
                                if(false || i % 5 == 2 || i == count-1 || publicVar.timer.intervalSafe(name: "recalcLayoutWhenReadInfo", second: 0.1)){
                                    recalcLayout(dir)
                                    // 立即刷新布局，让已完成尺寸计算的图片立刻以正确比例显示
                                    // Update layout immediately so processed items show correct aspect ratios
                                    collectionView.collectionViewLayout?.invalidateLayout()
                                    // collectionView.reloadData()
                                }
                                ...
                                if(dir == curFolder && keepScrollPos && i == count-1){
                                    ...
                                    collectionView.numberOfItems(inSection:0)
                                    // 恢复刷新前的滚动位置，防止新增文件导致跳转
                                    // Restore scroll position saved before refresh, prevents jumping to new files
                                    if let savedPos = refreshSavedScrollPos {
                                        mainScrollView.contentView.setBoundsOrigin(savedPos)
                                        mainScrollView.reflectScrolledClipView(mainScrollView.contentView)
                                        refreshSavedScrollPos = nil
                                    }
                                    setProgress(1.0)
                                }
        ...
        let thumbThread = Thread { [weak self] in
            ...
            while true{
                ...
                operationQueue.addOperation { [weak self] in
                    ...
                    if willTerminate {return}
                    loadImageTaskPool.lock.lock()
                    let firstTask = loadImageTaskPool.pop()
                    loadImageTaskPool.lock.unlock()
                    if let firstTask = firstTask {
                        // (dir,key,i,doNotActualRead)
                        fileDB.lock()
                        let dir=firstTask.0
                        let dirModel=firstTask.1
                    ...
                    let isExternal = VolumeManager.shared.isExternalVolume(key.path)
                    operationQueue.maxConcurrentOperationCount = isExternal ? globalVar.thumbThreadNum_External : globalVar.thumbThreadNum
...
//                                }
                                ...
                                if image != nil {
                                    ...
                                    DispatchQueue.main.async { [weak self] in
                                        ...
                                        if dirModel.isFiltered && !dirModel.aiOrderedPaths.isEmpty {
                                            displayIndex = dirModel.aiOrderedPaths.firstIndex(of: file.path) ?? i
                                        } else {
                                            displayIndex = i
                                        }
                                        fileDB.unlock()
        ...
        if(itemSorted.count>0){
            ...
            if let dirModel = fileDB.db[SortKeyDir(curFolder)] {
                for itemIndex in newRange {
                    let file = dirModel.fileForDisplay(atOffset: itemIndex)
                    if let file, file.image == nil, let key = dirModel.files.first(where: { $0.1.path == file.path })?.0 {
    ...
    func startWatchingDirectory(atPath path: String) {
        ...
        // 使用 FSEvents 监听目录变化（比 DispatchSourceFileSystemObject 更可靠）
        // Use FSEvents for directory monitoring (more reliable than DispatchSourceFileSystemObject)
        let callback: FSEventStreamCallback = { (_, clientInfo, _, eventPaths, _, _) in
            guard let info = clientInfo else { return }
            let vc = Unmanaged<ViewController>.fromOpaque(info).takeUnretainedValue()
            let _ = Unmanaged<CFArray>.fromOpaque(eventPaths).takeUnretainedValue() as! [String]
            
            if vc.publicVar.isInFileOperation {
                // 文件操作进行中，忽略事件
            } else {
                vc.scheduledRefresh()
            }
        ...
        watchEventStream = FSEventStreamCreate(
            kCFAllocatorDefault,
            callback,
            &context,
            pathsToWatch,
        ...
        if let stream = watchEventStream {
    ...
    func scheduledRefresh(){
        DispatchQueue.main.async { [weak self] in
            guard let self = self else { return }
            // 外置卷 expand 缓存随目录变化失效
            treeViewData.invalidateAllExpandCache()
            publicVar.fileChangedCount = 0
            // 保存当前滚动位置，防止刷新后跳转到新增文件
            // Save current scroll position to prevent jumping to new files after refresh
            refreshSavedScrollPos = mainScrollView.contentView.bounds.origin
            folderMonitorTimer?.invalidate()
            folderMonitorTimer = Timer.scheduledTimer(withTimeInterval: 0.2, repeats: false) { [weak self] _ in
                guard let self = self else { return }
                // 强制 keepScrollPos=false，让新文件能渐进插入 collectionView
                // Force keepScrollPos=false so new files are inserted progressively
                fileDB.lock()
                let refreshFolder = fileDB.curFolder
                fileDB.db[SortKeyDir(refreshFolder)]?.keepScrollPos = false
                fileDB.ver += 1
                fileDB.unlock()
                ...
                // 后台增量扫描：只处理新增/删除的文件，避免全量扫描 9576 个文件
                // Incremental scan: only process new/deleted files to avoid full scan
                DispatchQueue.global(qos: .userInitiated).async { [weak self] in
                    guard let self = self else { return }
                    guard let folderURL = URL(string: refreshFolder) else { return }

                    // 1. 快速读取目录内容（不带属性，仅文件名，很快）
                    // Fast directory read (file names only, no properties)
                    guard let currentURLs = try? FileManager.default.contentsOfDirectory(
                        at: folderURL, includingPropertiesForKeys: nil, options: []
                    ) else {
                        DispatchQueue.main.async { self.startWatchingDirectory(atPath: refreshFolder.replacingOccurrences(of: "file://", with: "").removingPercentEncoding!) }
                        return
                    }

                    // 2. 构建当前文件名字集合（排除 AppleDouble / .DS_Store）
                    var currentNames = Set<String>()
                    currentNames.reserveCapacity(currentURLs.count)
                    for u in currentURLs {
                        let name = u.lastPathComponent
                        if name.hasPrefix("._") || name == ".DS_Store" { continue }
                        currentNames.insert(name)
                    }

                    // 3. 获取 BTree 中的旧文件名字集合
                    fileDB.lock()
                    guard let dirModel = fileDB.db[SortKeyDir(refreshFolder)] else {
                        fileDB.unlock()
                        DispatchQueue.main.async { self.startWatchingDirectory(atPath: refreshFolder.replacingOccurrences(of: "file://", with: "").removingPercentEncoding!) }
                        return
                    }
                    var oldNames = Set<String>()
                    var nameToKey: [String: (SortKeyFile, FileModel)] = [:]
                    for (key, file) in dirModel.files {
                        if let fileURL = URL(string: file.path) {
                            let name = fileURL.lastPathComponent
                            oldNames.insert(name)
                            nameToKey[name] = (key, file)
                        }
                    }
                    let oldCount = dirModel.files.count
                    fileDB.unlock()

                    let newNames = currentNames.subtracting(oldNames)
                    let removedNames = oldNames.subtracting(currentNames)
                    var hasChanges = false

                    // 4. 处理被删除的文件
                    if !removedNames.isEmpty {
                        fileDB.lock()
                        if let model = fileDB.db[SortKeyDir(refreshFolder)] {
                            for name in removedNames {
                                if let (key, file) = nameToKey[name] {
                                    model.files.removeValue(forKey: key)
                                    GeoCache.shared.removeFile(path: file.path)
                                }
                            }
                        }
                        fileDB.unlock()
                    ...
                    // 5. 处理新增的文件（仅读取新文件的属性，跳过 9570+ 个未变化文件）
                    if !newNames.isEmpty {
                        let properties: [URLResourceKey] = [
                            .isHiddenKey, .isDirectoryKey, .fileSizeKey,
                            .contentModificationDateKey, .creationDateKey
                        ]
                        var newEntries: [(SortKeyFile, FileModel)] = []
                        newEntries.reserveCapacity(newNames.count)
                        ...
                        for name in newNames {
                            ...
                            let sortKey = SortKeyFile(filePath, isInSameDir: true,
                                                       sortType: publicVar.profile.sortType,
                                                       isSortFolderFirst: publicVar.profile.isSortFolderFirst,
                                                       isSortUseFullPath: publicVar.profile.isSortUseFullPath,
                                                       randomSeed: publicVar.randomSeed)
                            sortKey.size = fileSize ?? 0
                            if let md = modDate { sortKey.modDate = md }
                            if let cd = createDate { sortKey.createDate = cd }

                            let fileModel = FileModel(path: filePath, ver: fileDB.ver,
                                                       isDir: isDir, fileSize: fileSize,
                                                       createDate: createDate, modDate: modDate)
                            fileModel.isHidden = isHidden
                            if !isDir {
                                fileModel.ext = fileURL.pathExtension.lowercased()
                                if globalVar.HandledImageAndRawExtensions.contains(fileModel.ext) {
                                    fileModel.type = .image
                                } else if globalVar.HandledVideoExtensions.contains(fileModel.ext) {
                                    fileModel.type = .video
                                } else {
                                    fileModel.type = .other
                                }
                            } else {
                                fileModel.type = .folder
                            }
                            newEntries.append((sortKey, fileModel))
                        }

                        if !newEntries.isEmpty {
                            newEntries.sort { $0.0 < $1.0 }
                            fileDB.lock()
                            if let model = fileDB.db[SortKeyDir(refreshFolder)] {
                                for (key, file) in newEntries {
                                    model.files[key] = file
                                }
                            }
                            fileDB.unlock()
                            hasChanges = true
                    ...
                    if let model = fileDB.db[SortKeyDir(refreshFolder)] {
                        var id = 0
                        for (_, file) in model.files {
                            file.ver = model.ver
                            file.canBeCalcued = false
                            file.id = id
                            id += 1
                        }
                    }
                    ...
                    // 更新 DirMetadataCache
                    if let cacheURL = URL(string: refreshFolder) {
                        let allNames = Array(currentNames)
                        DirMetadataCache.shared.updateSnapshot(cacheURL, fileNames: allNames, isShowHiddenFile: publicVar.isShowHiddenFile)
                    }

                    // 8. 主线程更新视图
                    DispatchQueue.main.async { [weak self] in
                        guard let self = self else { return }
                        switchFolder(path: refreshFolder)
                        startWatchingDirectory(atPath: refreshFolder.replacingOccurrences(of: "file://", with: "").removingPercentEncoding!)
                    }
                }
            }
        }
